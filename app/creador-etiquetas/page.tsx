    ...
    <div className="min-h-screen overflow-hidden bg-[#080b0d] text-white selection:bg-orange-500/30">
      ...
      <main className="relative mx-auto max-w-7xl px-5 pb-16 pt-10 sm:px-8 sm:pt-14">
        ...
        <section className="mx-auto max-w-3xl text-center">
          ...
          <p className="mx-auto mt-5 max-w-2xl text-base leading-7 text-slate-400 sm:text-lg">CA46 registra todos los datos de trazabilidad que encuentre en la factura y te permite revisarlos antes de publicar.</p>
        </section>
        ...
        <section className="mx-auto mt-8 max-w-5xl rounded-[2rem] border border-white/10 bg-white/[.035] p-5 sm:p-7">
          <div className="flex flex-wrap items-end justify-between gap-4 border-b border-white/10 pb-5"><div><p className="text-xs font-black uppercase tracking-[.22em] text-orange-400">Paso 1 · Fotografías</p><h2 className="mt-2 text-2xl font-black sm:text-3xl">{photos.length === 0 ? 'Aún no has añadido ninguna factura' : `${photos.length} ${photos.length === 1 ? 'fotografía seleccionada' : 'fotografías seleccionadas'}`}</h2>{photos.length > 0 ? <p className="mt-2 text-sm font-semibold text-slate-500">{(totalSize / 1024 / 1024).toFixed(1)} MB originales · CA46 las optimiza antes de leerlas</p> : null}</div>{photos.length > 0 && !analyzing && !publishing ? <button type="button" onClick={clearAll} className="rounded-full border border-white/10 px-4 py-2 text-sm font-black text-slate-400">Quitar todas</button> : null}</div>
        ...
        {results.length > 0 ? <section className="mx-auto mt-8 max-w-6xl">
          ...
          <div className="mt-6 space-y-7">{results.map((invoice, invoiceIndex) => <article key={invoice.photoId} className="overflow-hidden rounded-[2rem] border border-white/10 bg-white/[.035]">
            ...
            <div className="space-y-5 p-5 sm:p-6">{invoice.labels.length === 0 ? <div className="rounded-2xl border border-amber-400/20 bg-amber-400/[.06] p-5 text-amber-100/80">No se ha podido separar ninguna partida.</div> : invoice.labels.map((label, labelIndex) => <div key={`${invoice.photoId}-${labelIndex}`} className={`rounded-2xl border p-5 ${label.needs_review ? 'border-amber-400/25 bg-amber-400/[.035]' : 'border-white/10 bg-black/20'}`}>
              ...
              {/descongelad/i.test(label.frescura || '') ? <div className="mt-4 rounded-xl border border-amber-400/25 bg-amber-400/10 px-4 py-3 text-sm font-black uppercase tracking-wide text-amber-200">⚠ Consumir preferentemente en 3 días</div> : null}
          ...
          <div className="mt-7 rounded-[2rem] border border-white/10 bg-white/[.035] p-6 sm:flex sm:items-center sm:justify-between sm:gap-6"><div><p className="text-xs font-black uppercase tracking-[.2em] text-orange-400">Paso 3</p><h3 className="mt-2 text-2xl font-black">Publicar durante 72 horas</h3><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Después de publicar, CA46 se queda aquí para que puedas analizar otra factura.</p>{!allReviewed ? <p className="mt-3 text-sm font-bold text-amber-300">Revisa y confirma todas las etiquetas antes de publicar.</p> : null}{publishError ? <p className="mt-3 text-sm font-bold text-rose-300">{publishError}</p> : null}</div><button type="button" onClick={publishLabels} disabled={!allReviewed || publishing} className="mt-5 w-full rounded-2xl bg-orange-500 px-6 py-4 font-black text-[#111416] disabled:bg-slate-800 disabled:text-slate-600 sm:mt-0 sm:w-auto">{publishing ? 'Publicando…' : `Publicar ${totalLabels} ${totalLabels === 1 ? 'etiqueta' : 'etiquetas'}`}</button></div>
        </section> : null}
      </main>
    </div>
  );
