...
export default function MiEmpresaPage() {
  ...
  useEffect(() => setConfig(loadCompanyConfig()), []);
  ...
  async function prepareDrive() {
    ...
    try {
      const response = await fetch('/api/company-drive', {
        ...
        body: JSON.stringify({ companyName: config.businessName }),
      });
          ...
          <div className="space-y-6">
            <section className="rounded-[2rem] border border-white/10 bg-white/[.035] p-6">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                ...
                ].map(([key,label]) => <label key={key}><span className="mb-2 block text-[11px] font-black uppercase tracking-[.16em] text-slate-500">{label}</span><input value={String(config[key as keyof CompanyConfig] || '')} onChange={(e) => patch(key as keyof CompanyConfig, e.target.value as never)} className={inputClass} /></label>)}
              </div>
            </section>
            ...
            <section className="rounded-[2rem] border border-white/10 bg-white/[.035] p-6">
              ...
              <p className="mt-4 text-sm text-slate-500">CA46 no guarda usuarios, claves, IBAN ni credenciales bancarias. Solo muestra accesos a la web oficial.</p>
            </section>
            ...
            <section className="rounded-[2rem] border border-white/10 bg-white/[.035] p-6">
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-2">
                <div className="rounded-2xl border border-white/10 bg-black/20 p-5">
                  ...
                  <p className="mt-3 text-sm leading-6 text-slate-400">Crea la carpeta de empresa con Facturas, Etiquetas e Histórico.</p>
                  <div className="mt-4 flex flex-wrap gap-2">
                    <button type="button" onClick={prepareDrive} disabled={driveLoading} className="rounded-xl bg-orange-500 px-4 py-3 text-sm font-black text-[#111416] disabled:opacity-50">{driveLoading ? 'Preparando…' : config.driveConnected ? 'Revisar carpeta' : 'Crear carpeta CA46'}</button>
            ...
            <section className="rounded-[2rem] border border-white/10 bg-white/[.035] p-6">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                ...
                <label><span className="mb-2 block text-[11px] font-black uppercase tracking-[.16em] text-slate-500">Horas visibles</span><input type="number" min={1} value={config.labelsHours} onChange={(e) => patch('labelsHours', Number(e.target.value || 72))} className={inputClass}/></label>
            ...
            <button type="button" onClick={handleSave} className="w-full rounded-2xl bg-orange-500 px-6 py-4 text-lg font-black text-[#111416]">{saved ? '✓ Guardado' : 'Guardar Mi empresa'}</button>
          </div>
